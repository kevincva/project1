...
aws cloudformation create-stack --stack-name KCAppStack-`date +%m%d%Y`-`date +%H%M%S` --template-body https://s3.amazonaws.com/devopstechtalkleidos-r/KCApp/CFN-KCHelloApp.yml --parameters ParameterKey=KeyName,ParameterValue=kevinchinekypair ParameterKey=Mission,ParameterValue=myDEV ParameterKey=NameTagValue,ParameterValue=KCApp-`date +%m%d%Y`-`date +%H%M%S` ParameterKey=Network,ParameterValue=myNetwork ParameterKey=ProjectTag,ParameterValue=KCApp ParameterKey=Tier,ParameterValue=DEV --region=us-east-1
